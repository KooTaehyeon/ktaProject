...
function openThisYearMoviesWindow() {
  const thisYearWindow = window.open(
    '',
    'ThisYearMovies',
    'width=800,height=600'
  );
  thisYearWindow.document.write('<html><head><title>올해의 영화</title>');
  thisYearWindow.document.write(
    '<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">'
  );
  thisYearWindow.document.write(
    '</head><body><div class="container mt-4"><h2>올해의 영화 로딩 중...</h2></div></body></html>'
  );
...
async function getThisYearMovies(targetWindow) {
  ...
  try {
    ...
    targetWindow.document.body.innerHTML =
      '<div class="container mt-4"><p class="text-danger">올해의 영화를 가져오는 중 오류가 발생했습니다.</p></div>';
  }
}
  ...
  const movieList = movies
    .map(
      (movie, index) => `
        ...
        movie.poster_path
      }" class="img-fluid rounded-start" alt="${movie.title} 포스터">
                </div>
                <div class="col-md-8">
                    <div class="card-body">
                        ...
                        <p class="card-text">${movie.overview.slice(
                          0,
                          100
                        )}...</p>
                    </div>
...
function openTopRatedWindow() {
  const topRatedWindow = window.open(
    '',
    'TopRatedMovies',
    'width=800,height=600'
  );
  topRatedWindow.document.write('<html><head><title>평점 Top 10 영화</title>');
  topRatedWindow.document.write(
    '<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">'
  );
  topRatedWindow.document.write(
    '</head><body><div class="container mt-4"><h2>평점 Top 10 영화 로딩 중...</h2></div></body></html>'
  );
...
async function getTopRatedMovies(targetWindow) {
  try {
    ...
    targetWindow.document.body.innerHTML =
      '<div class="container mt-4"><p class="text-danger">Top 10 영화를 가져오는 중 오류가 발생했습니다.</p></div>';
  }
}
...
async function searchMovies() {
  ...
  try {
    ...
    resultDiv.innerHTML =
      '<p class="text-danger">영화 검색 중 오류가 발생했습니다. 다시 시도해주세요.</p>';
  }
}
...
function displayResults(movies) {
    ...
    <div class="card mb-3">
      <div class="row g-0">
        <div class="col-md-4">
          ${
            movie.poster
              ? `<img src="${movie.poster}" class="img-fluid rounded-start movie-poster" alt="${movie.movieNm} 포스터" data-tmdb-id="${movie.tmdbId}">`
  ...
  document.querySelectorAll('.movie-poster').forEach((poster) => {
    poster.addEventListener('click', () =>
      showMovieDetails(poster.dataset.tmdbId)
    );
  });
}
        ...
        ${
          movie.reviews.results.length > 0
            ...
            <div class="review mb-2">
              <p><strong>${review.author}</strong>: ${review.content.slice(
                    0,
                    100
                  )}...</p>
            </div>
